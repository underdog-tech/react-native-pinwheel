...
- (void)initPinwheelWrapperVC {
    ...
    if (self.token != nil) {
        self.pinwheelWrapperVC = [[PinwheelWrapperVC alloc] initWithToken:self.token delegate:self sdk:@"react native" version:@"3.3.2" useDarkMode:self.useDarkMode useAppBoundDomains:YES useAppBoundDomainsForNativeLink:YES];
        [self addSubview:self.pinwheelWrapperVC.view];
    }
}
